...
class VoiceProfileDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.is_default:
            other_profile = (
                VoiceProfile.objects.filter(user=request.user, is_active=True)
...
def set_default_voice_profile(request, profile_id):
    ...
    try:
        voice_profile = VoiceProfile.objects.get(
            id=profile_id, user=request.user, is_active=True
        )

        # Remove default from other profiles
        VoiceProfile.objects.filter(user=request.user, is_default=True).update(
            is_default=False
        )
        ...
        return Response(
            {"error": "Voice profile not found"}, status=status.HTTP_404_NOT_FOUND
        )
