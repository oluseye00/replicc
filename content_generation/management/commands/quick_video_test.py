...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🧪 Quick Video Generation Test')
        )
        ...
        try:
            ...
            if os.path.exists(test_video_path) and os.path.getsize(test_video_path) > 0:
                ...
                self.stdout.write(self.style.ERROR('❌ Video file was not created or is empty'))
        ...
        self.stdout.write('   The full video generation may take 30-120 seconds depending on content.')
        self.stdout.write('\n🚀 To test full video generation:')
        self.stdout.write('   python manage.py test_video --sync --duration=3 "Short test"')