...
from content_generation.video_tasks import generate_video_from_text, create_video_template
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('🎬 Testing Video Generation')
        )
        ...
        self.stdout.write(f'📐 Resolution: {resolution[0]}x{resolution[1]} ({resolution_type})')
    ...
    def _test_text_video(self, text, title, user_id, style, duration, resolution, wait_for_result):
            ...
            if wait_for_result:
                ...
                try:
                    ...
                    if response['success']:
                        ...
                        self.stdout.write(f'⏱️  Duration: {response["duration"]} seconds')
                        self.stdout.write(f'📐 Resolution: {response["resolution"][0]}x{response["resolution"][1]}')
                        self.stdout.write(f'🎨 Style: {response["style"]}')
                        self.stdout.write(f'💾 File size: {response["file_size_mb"]:.2f} MB')
                        self.stdout.write(f'📄 Text chunks: {response["text_chunks_count"]}')
                        self.stdout.write('='*60)
    ...
    def _test_template_video(self, text, title, user_id, wait_for_result):
        ...
        slides_data = [
            {
                "text": "Welcome to Template Videos!",
                "duration": 3
            },
            {
            ...
            {
                "text": "Created with AI-powered video generation",
                "duration": 3
            }
        ]
