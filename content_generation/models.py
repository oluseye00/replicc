...
class ContentGeneration(models.Model):
    ...
    voice_profile = models.ForeignKey(
        "accounts.VoiceProfile", on_delete=models.SET_NULL, null=True, blank=True
    )
...
class SavedContent(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="saved_content"
    )
    content_generation = models.ForeignKey(
        ContentGeneration, on_delete=models.CASCADE, related_name="saves"
    )
    ...
    def __str__(self):
        return (
            f"{self.user.email} saved content from {self.content_generation.created_at}"
        )
...
class ContentFeedback(models.Model):
    ...
    content_generation = models.OneToOneField(
        ContentGeneration, on_delete=models.CASCADE, related_name="feedback"
    )
    ...
    voice_match_rating = models.IntegerField(
        choices=RATING_CHOICES, null=True, blank=True
    )
