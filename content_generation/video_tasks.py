...
def generate_video_from_text(
    ...
    try:
        ...
        saved_path = default_storage.save(
            f"generated_videos/{output_filename}", ContentFile(video_content)
        )
...
def create_video_template(
    ...
    try:
        ...
        final_video.write_videofile(
            temp_path, fps=24, codec="libx264", verbose=False, logger=None
        )
...
def _create_slideshow_clips(
    text_chunks, slide_duration, resolution, bg_color, text_color, font_size
):
    ...
    for text in text_chunks:
        ...
        try:
            ...
            text_clip = (
                ImageClip(temp_path).set_duration(slide_duration).set_position("center")
            )
...
def _create_text_image(
    text, width, height, font_size=40, text_color=(255, 255, 255), bg_color=(30, 30, 50)
):
...
def _create_template_slide(text, image_path, duration, style, slide_number):
    ...
    background = ColorClip(
        size=(1080, 1920), color=style_config["bg_color"], duration=duration
    )
