...
def generate_platform_content(request, generation_id):
    ...
    generation = get_object_or_404(
        ContentGeneration, id=generation_id, user=request.user
    )

    platform = request.data.get("platform")
    if not platform:
        return Response(
            {"error": "Platform is required"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    return Response(
        {"platform": platform, "content": content, "generation_id": str(generation.id)}
    )
