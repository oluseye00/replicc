...
class VideoAsset(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="video_assets"
    )
...
class ContentVideoMatch(models.Model):
    ...
    video_asset = models.ForeignKey(
        VideoAsset, on_delete=models.CASCADE, related_name="content_matches"
    )
    ...
    editing_suggestions = models.JSONField(
        default=list
    )  # How to combine content + video
...
class MatchingRule(models.Model):
    ...
    weight_platform = models.FloatField(
        default=0.2
    )  # How important is platform matching
...
class VideoProcessingQueue(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    video_asset = models.ForeignKey(
        VideoAsset, on_delete=models.CASCADE, related_name="processing_tasks"
    )
