...
from .models import ContentVideoMatch, MatchingRule, VideoAsset
...
class VideoAnalyzer:
    ...
    def _analyze_visual_elements(self, video_asset: VideoAsset) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "estimated_motion": np.mean(motion_estimates)
                if motion_estimates
                else 0,
                "visual_complexity": np.mean(color_diversity),
                "lighting_quality": "good"
                if np.mean(brightness_values) > 50
                else "poor",
            }
    ...
    def _analyze_scenes_and_pace(self, video_asset: VideoAsset) -> Dict:
        ...
        try:
            ...
            duration = frame_count / fps
            cuts_per_minute = (
                (len(scene_changes) / duration) * 60 if duration > 0 else 0
            )
    ...
    def _ai_style_analysis(
        self, video_asset: VideoAsset, analysis_results: Dict
    ) -> Dict:
...
Please provide your analysis in JSON format."""
        ...
        try:
            ...
            video_asset.pace = analysis_results.get("scenes", {}).get(
                "estimated_pace", ""
            )
            video_asset.style = ai_analysis.get("style", "")
            video_asset.suitable_platforms = ai_analysis.get(
    ...
    def _create_video_description(self, analysis_results: Dict) -> str:
        ...
        if scenes.get("cuts_per_minute"):
            description_parts.append(
                f"Editing pace: {scenes['cuts_per_minute']:.1f} cuts per minute"
            )
...
class ContentVideoMatcher:
    ...
    def find_matches(
        self, content_generation, limit: int = 5
    ) -> List[ContentVideoMatch]:
        """Find the best video matches for generated content"""

        user = content_generation.user
        available_videos = VideoAsset.objects.filter(user=user, is_processed=True)
            ...
            if match_score > 0.3:  # Minimum threshold
                match = ContentVideoMatch.objects.create(
                    ...
                    match_reasoning=self._generate_match_reasoning(
                        content_generation, video, match_score
                    ),
                )
    ...
    def _calculate_match_score(
        self, content_generation, video_asset: VideoAsset
    ) -> float:
        ...
        technical_score = self._calculate_technical_score(
            content_generation, video_asset
        )
    ...
    def _calculate_technical_score(
        ...
        if (
            "tiktok" in content_generation.target_platforms
            or "instagram" in content_generation.target_platforms
        ):
    ...
    def _infer_content_type(self, content_generation) -> str:
        ...
        if any(word in topic for word in ["tutorial", "how to", "step by step"]):
            ...
            word in topic for word in ["learn", "education", "explain", "understand"]
        ):
            return "educational"
        else:
            return "general"
    ...
    def _generate_match_reasoning(
        self, content_generation, video_asset: VideoAsset, score: float
    ) -> str:
